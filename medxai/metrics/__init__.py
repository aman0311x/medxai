...
def _validate_inputs(pred: torch.Tensor, target: torch.Tensor) -> None:
    ...
    if pred.shape != target.shape:
        raise ValueError(f"Shape mismatch: pred shape {pred.shape} must match target shape {target.shape}")
    if pred.device != target.device:
        raise ValueError(f"Device mismatch: pred is on {pred.device} but target is on {target.device}")
